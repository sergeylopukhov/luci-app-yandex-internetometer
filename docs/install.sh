#!/bin/sh
# luci-app-yandex-internetometer installer 1.0.12
set -eu
...
while [ "$#" -gt 0 ]; do case "$1" in --yes) YES=1;; --force) FORCE=1;; --version) printf '1.0.12\n'; exit 0;; --help) help; exit 0;; *) die "неизвестный аргумент: $1";; esac; shift; done
...
rm -f /tmp/luci-indexcache.* 2>/dev/null || true; [ ! -x /etc/init.d/rpcd ] || /etc/init.d/rpcd reload >/dev/null 2>&1 || true; [ -x /usr/bin/yandex-internetometer ] || die 'backend не установлен'; /usr/bin/yandex-internetometer status | grep -q '"ok"' || die 'backend status не вернул JSON'; ACTUAL="$(installed_version || true)"; [ "$ACTUAL" = "$VERSION-r1" ] || die "установлена версия ${ACTUAL:-неизвестна}, ожидалась $VERSION-r1; повторите через несколько минут"; say "Установлена версия $VERSION. LuCI → Статус → Интернетометр." "Installed version $VERSION. LuCI → Status → Internetometer."
