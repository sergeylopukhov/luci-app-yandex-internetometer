#!/bin/sh
# luci-app-yandex-internetometer installer 1.1.0
set -eu
...
while [ "$#" -gt 0 ]; do case "$1" in --yes) YES=1;; --force) FORCE=1;; --version) printf '1.1.0\n'; exit 0;; --help) help; exit 0;; *) die "неизвестный аргумент: $1";; esac; shift; done
...
[ ! -f /etc/config/yandex-internetometer ] || { BACKUP="/etc/config/yandex-internetometer.backup.$(date +%Y%m%d%H%M%S)"; cp /etc/config/yandex-internetometer "$BACKUP"; chmod 600 "$BACKUP" 2>/dev/null || true; }
