...
- Новые установки по умолчанию: 6 потоков в обе стороны, HTTP, 15/25 секунд, 60 замеров задержки, payload 8 000 000 байт; исходящий тест включён, отладка выключена.
- Диапазон замеров задержки: от 25 до 120.

## v1.0.13

- Подписи параметров в настройках получили отдельный левый отступ: 20 px на широком экране и 14 px на узком. Убрана вложенная карточка настроек: остаются только раскрывающийся раздел и строки параметров.

## v1.0.12

- Убран повторяющийся заголовок «Настройки» внутри раскрывающегося блока.

## v1.0.11

- В настройках восстановлен единый внутренний отступ: подписи полей не прилипают к краю карточки.

## v1.0.10

- Во время замера все числовые метрики выделяются красным и остаются читаемыми в тёмной теме.
- При переходе к upload отменяется активная анимация download: финальная входящая скорость сразу фиксируется.

## v1.0.9

- Блок результатов полностью оформлен для тёмной темы: контрастные фон, подписи, значения и границы.

## v1.0.8

- Завершённая метрика теперь мгновенно фиксируется на фактическом значении: download не продолжает сглаживаться во время upload.

## v1.0.7

- Исправлен контраст подписей настроек в светлой теме.
- Добавлен переключатель светлой и тёмной темы интерфейса; выбор сохраняется в браузере.

## v1.0.6

- Промежуточные цифры скорости и дуга индикатора анимируются непрерывно между секундными обновлениями backend.
- Для живых значений добавлено EMA-сглаживание; итоговый результат остаётся точным, без сглаживания.

## v1.0.5

- Настройки LuCI переработаны: контрастные поля, читаемые подписи и подсказки, мобильная раскладка.
- Установщик теперь подтверждает фактически установленную версию пакета и не сообщает об успехе при устаревшем индексе APK.

## v1.0.4

- Исправлена проверка `version.json` на OpenWrt jq без ONIGURUMA; LuCI снова показывает доступную версию.
- В уведомлении LuCI добавлена готовая команда обновления.
- Stop корректно завершает worker и дочерние curl-процессы.

## v1.0.3

- Исправлен latency warmup: теперь выполняется начальный запрос и заданное число учитываемых probe-запросов.

## v1.0.2

- Исправлена проверка безопасного Yandex CDN upload URL: поддержан актуальный ожидаемый путь `/upload?...&size=30720`.

## v1.0.1

- Исправлена безопасная обработка значений UCI и CLI без динамического выполнения shell-кода.
- APK-репозиторий GitHub Pages теперь публикуется вместе с подписанными `packages.adb` из той же сборки; старые APK удаляются.
